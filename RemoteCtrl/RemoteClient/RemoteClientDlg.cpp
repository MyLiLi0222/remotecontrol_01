...
END_MESSAGE_MAP()//ASKAI 这里定义的是什么？？？？
...
int CRemoteClientDlg::SendCommandPack(int nCmd, bool bAutoClose, BYTE* pData, size_t nLength)
{
	...
	if (!ret) {
		AfxMessageBox("初始化网络失败！");
		return -1;
	}

	CPacket pack(nCmd,pData,nLength);
	ret = pClient->Send(pack);
	TRACE(" Send ret =  %d, SendPacket \r\n", ret);
...
void CRemoteClientDlg::threadEntryForWathcData(void* arg)
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		do {
			...
			while (nCont < nLength) {
				...
				nCont += pClient->GetPacket().strData.size();
...
void CRemoteClientDlg::LoadFileCurrent()
{
	...
	//CClientSocket::getInstance()->GetPacket().Data();
...
{	
	...
	while (pInfo->HasNext) {
		TRACE("[%s] is dir %d\r\n", pInfo->szFileName, pInfo->IsDirectory);
		if (pInfo->IsDirectory) {
			if (CString(pInfo->szFileName) == "." || (CString(pInfo->szFileName) == "..")) {//ASKAI 这一段的逻辑
				int cmd = pClient->DealCommand();
				TRACE("ACK : %d \r\n",cmd);
				if (cmd < 0) {
					break;
...
{
	HTREEITEM hSub = NULL;//HTREEITEM具体是什么？ASKAI
	do {
...
LRESULT CRemoteClientDlg::OnSendPacket(WPARAM wParam, LPARAM lParam)
{
	...
	case 4: {
		...
		ret = SendCommandPack(cmd, wParam & 1, (BYTE*)(LPCSTR)strFile, strFile.GetLength());
	}
		  break;
	case 5: {//鼠标操作
		ret = SendCommandPack(cmd, wParam & 1, (BYTE*)lParam, sizeof(MOUSEEVENT));
	}
		  break;
	case 6:
	case 7:
...
void CRemoteClientDlg::OnBnClickedBtnStartWatch()
{
	...
	HANDLE hThread = (HANDLE)_beginthread(CRemoteClientDlg::threadEntryForWathcData, 0, this);
	dlg.DoModal();
	m_isClosed = true;
