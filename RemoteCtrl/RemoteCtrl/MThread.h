...
public:
	...
	bool Start() {
		m_bStatus = true;
		m_hThread = (HANDLE)_beginthread(&MThread::ThreadEnty, 0, this);
		if (!IsValid()) {
			m_bStatus = false;
		}
		return m_bStatus;
...
private:
		...
		while(m_bStatus) {
			...
			if (worker.IsValid()) {
				if (WaitForSingleObject(m_hThread, 0) == WAIT_TIMEOUT) {
					...
					if (ret != 0) {
						CString str;
						str.Format(_T("thread found waring code %d\r\n"), ret);
	...
	static void ThreadEnty(void* arg) {
		MThread* thiz = (MThread*)arg;
		if (thiz) {
			thiz->ThreadWorker();
		}
		_endthread();
...
class MThreadPool {//线程池
public:
	MThreadPool(size_t size) {
		m_threads.resize(size);
		for (size_t i = 0; i < size; i++) {
			m_threads[i] = new MThread();
		}
	}
	MThreadPool() {}
	~MThreadPool() {
